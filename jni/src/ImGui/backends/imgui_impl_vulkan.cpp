...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    // [本地修改] 字体图集按单通道(R8)上传，不转 RGBA32。
    // RGBA32 版每个像素都是 (255,255,255,a)，RGB 三个通道纯属重复：2048x2048 图集占 16MB 显存，
    // 单通道只要 4MB。下面图像视图用分量映射把 RGB 固定为 1、A 取 R，
    // 着色器采样结果仍是 (1,1,1,a)，与原来逐位相同。
    // R8_UNORM 作采样纹理 + 线性过滤是 Vulkan 规范强制支持的格式；图集里没有彩色像素(不用 FreeType 彩色字形)
    ...
    // Create the Image View:
    {
        ...
        info.format = VK_FORMAT_R8_UNORM;
        info.components.r = VK_COMPONENT_SWIZZLE_ONE;   // 单通道 -> (1,1,1,a)，见上面上传处的说明
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    // Create Swapchain
    {
        ...
        // [本地修改] 一律用调用方给的尺寸，不用 currentExtent。
        // Android 在显示相对原生方向转 90° 时会把 currentExtent 的宽高对调（让应用自己做预旋转），
        // 表面是正方形时看不出来，改成横屏 3200x1440 后会被对调成 1440x3200、画面错位。
        // preTransform 仍是 IDENTITY，缓冲坐标 = 逻辑(横屏)坐标。Android 允许尺寸与 currentExtent 不同。
        uint32_t ext_w = (uint32_t)w, ext_h = (uint32_t)h;
        if (ext_w < cap.minImageExtent.width)  ext_w = cap.minImageExtent.width;
        if (ext_h < cap.minImageExtent.height) ext_h = cap.minImageExtent.height;
        if (cap.maxImageExtent.width && ext_w > cap.maxImageExtent.width)   ext_w = cap.maxImageExtent.width;
        if (cap.maxImageExtent.height && ext_h > cap.maxImageExtent.height) ext_h = cap.maxImageExtent.height;
        info.imageExtent.width = wd->Width = (int)ext_w;
        info.imageExtent.height = wd->Height = (int)ext_h;
        err = vkCreateSwapchainKHR(device, &info, allocator, &wd->Swapchain);
        check_vk_result(err);
        err = vkGetSwapchainImagesKHR(device, wd->Swapchain, &wd->ImageCount, nullptr);
        check_vk_result(err);
        printf("[交换链] 请求=%dx%d currentExtent=%ux%u currentTransform=0x%x 范围=%ux%u~%ux%u 实际=%ux%u 张数=%u 结果=%d\n",
               w, h, cap.currentExtent.width, cap.currentExtent.height, (unsigned)cap.currentTransform,
               cap.minImageExtent.width, cap.minImageExtent.height, cap.maxImageExtent.width, cap.maxImageExtent.height,
               ext_w, ext_h, wd->ImageCount, (int)err);
        fflush(stdout);
